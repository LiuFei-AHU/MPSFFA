"""
 This is a collection of tools for image augmentation when training the model.
...
class contrast(DataAug):
    def __init__(self, mn=0.8, mx=1.3, rate=0.5):
        super().__init__()
        self.mn = mn
        self.mx = mx
        self.rate = rate
    ...
    def __call__(self, data):
        if random.random() >= self.rate:
            data = np.clip(random.uniform(self.mn, self.mx) * data, -1, 1)
        return data
